...
while(cap.isOpened()):
	...
	# Generating mask to detect red color(skin)
	lower_red = np.array([-1,50,70])
	upper_red = np.array([19,256,255])
	...
	#upper_red = np.array([20,256,255])
	#mask1 = cv2.inRange(hsv,lower_red,upper_red)

	#lower_red = np.array([180,80,10])
